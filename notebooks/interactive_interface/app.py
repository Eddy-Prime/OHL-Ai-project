...
def chat():
    ...
    # ── Step 8: Promotion → Predict ───────────────────────────────────────────
        ...
        if opponent.lower() in SELLOUT_OPPONENTS:
            ...
            raw_low  = max(0,                round(np.exp(log_pred - std_dev)))
            raw_high = min(STADIUM_CAPACITY, round(np.exp(log_pred + std_dev)))
            ...
            low  = max(0,                round(np.exp(log_pred - std_dev)))
            high = min(STADIUM_CAPACITY, round(np.exp(log_pred + std_dev)))
